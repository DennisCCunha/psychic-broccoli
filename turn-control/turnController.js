...
class TurnController {
    ...
    handleSequenceClick(event) {
        ...
        const player = this.findPlayer(actionButton.dataset.playerId);
        if (!player) return;

        const action = actionButton.dataset.action;
        if (action === "toggle-turn") this.endTurn(player);
        if (action === "remove-player") this.removePlayer(player);
        if (action === "score" && Number.isFinite(Number(actionButton.dataset.delta))) {
            this.changeScore(player, Number(actionButton.dataset.delta));
        }
    ...
    handleListClick(event) {
        ...
        const player = this.findPlayer(button.dataset.playerId);
        if (!player) return;
        if (button.classList.contains("end-turn-button")) this.endTurn(player);
        if (button.classList.contains("remove-player-button")) this.removePlayer(player);
        if (button.classList.contains("score-button")) {
            this.changeScore(player, Number(button.dataset.delta));
        }
        this.renderPlayerList();
    }
    ...
    render() {
        return this.players.map((player) => `<div class="turn-player${player.run ? "" : " turn-player--ended"}" draggable="${!this.orderLocked}" data-player-id="${player.id}">
...
class TurnControllerElement extends HTMLElement {
    connectedCallback() {
