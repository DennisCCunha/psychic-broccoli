...
export class VoiceChat {
	constructor(peerManager) {
		...
		this.onStateChange = () => {};
		this.onPeerActivity = () => {};
		listenToPeerManager(peerManager, 'track', (event, peerId) => this._handleRemoteTrack(event, peerId));
	}

	async start() {
		this.localStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
		for (const track of this.localStream.getAudioTracks()) this.peerManager.addLocalTrack(track, this.localStream);
	...
	setMuted(muted) {
		this.muted = muted;
		if (this.localStream) for (const track of this.localStream.getAudioTracks()) track.enabled = !muted;
	...
	getPeerStates() {
		return [...this.peers.entries()].map(([peerId, peer]) => ({ peerId, speaking: peer.speaking ?? false }));
	}
	...
	_pollActivity(peerId, peer, isLocal = false) {
		...
		const tick = () => {
			...
			let sum = 0;
			for (const value of buf) sum += Math.abs(value - 128);
